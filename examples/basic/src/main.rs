...
fn run() -> Result<()> {
    ...
    let nowner_2 = Owner::new("Jane Doe")
        .set_date_of_birth(chrono::UTC.ymd(1985, 9, 8).and_hms(0, 0, 0));
    let funds_1 = Money::of_major(currency::SEK, 100);
    let funds_2 = Money::of_major(currency::SEK, 100);

    let conn =
        diesel_conn::establish_connection(None).chain_err(|| "Failed to establish connection")?;
    println!("All users are: {:?}", diesel_conn::all_accounts(&conn));


    let owner_1 =
        diesel_conn::add_owner(&conn, nowner_1)
        .chain_err(|| "Failed to add new owner to database")?;
    let nacc_1 =
        NewAccount::new(&owner_1, funds_1, password_1)
        .chain_err(|| "Failed to create new account")?;
    let owner_2 =
        diesel_conn::add_owner(&conn, nowner_2)
        .chain_err(|| "Failed to add new owner to database")?;
    let nacc_2 =
        NewAccount::new(&owner_2, funds_2, password_2)
        .chain_err(|| "Failed to create new account")?;
    let mut acc_1 =
        diesel_conn::add_account(&conn, nacc_1)
        .chain_err(|| "Failed to add new account to database")?;
    let mut acc_2 =
        diesel_conn::add_account(&conn, nacc_2)
        .chain_err(|| "Failed to add new account to database")?;
