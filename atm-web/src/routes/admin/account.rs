...
fn show_accounts(_admin: AdminUser, conn: pool::Conn) -> Result<Template> {
    let context = AccountsContext {
        accounts: interface::diesel_conn::all_accounts(&conn)
            ?
            .into_iter()
            .map(|acc| make_account_context(&conn, &acc.id).expect("FIX THIS, SHOULDN'T HAPPEN"))
            .collect(),
    };
...
pub fn make_account_context(conn: &pool::Conn, account_id: &Uuid) -> Result<AccountContext> {
    ...
    let transactions = interface::diesel_conn::transactions_from(&conn, &account)
        ...
        .into_iter()
        .filter_map(|trans| {
            Some(TransactionOfUser {
                serial: trans.serial(),
                // FIXME: Handle error properly, can fail if transaction is invalid.
                amount: format!("{}",
                                match trans.amount_as_money() {
                                    Ok(s) => s,
                                    Err(e) => {
    ...
    let context = AccountContext {
        funds: currency::convert_map_to_money(account.funds(&conn)?)
            .into_iter()
