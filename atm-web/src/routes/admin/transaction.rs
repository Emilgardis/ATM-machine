...
pub fn create_datetime_from_pg(offset: i64) -> chrono::DateTime<chrono::UTC> {
    chrono::DateTime::from_utc(
        pg_epoch().checked_add_signed(chrono::Duration::microseconds(offset)).unwrap(),
        chrono::UTC)
}
...
impl From<NewTransactionFromForm> for transaction::NewTransaction {
    fn from(trans: NewTransactionFromForm) -> transaction::NewTransaction {
        transaction::NewTransaction {
            ...
            date: trans.date.map(|offset| create_datetime_from_pg(offset)).unwrap_or(chrono::UTC::now()), // Not recommended but better than crash :-)
        }
    }
}

#[post("/admin-panel/post-transaction", data="<transaction>")]
pub fn post_transaction(_admin: AdminUser, conn: pool::Conn, transaction: Form<NewTransactionFromForm>) -> Result<Redirect, Flash<Redirect>> {
